...
def build_group_mr_details(cve: str, vulns: list, jira_ticket: str, confidence: float | None = None) -> dict:
...
def fix_group(ctx, cve, jira_ticket, severity, dry_run, force, local_only):
    ...
    # Fix the group
    fix_result = agent.fix_cve_group(cve, group, repo_path, dry_run=dry_run)
    ...
    if dry_run:
        mr_details = build_group_mr_details(cve, group, jira_ticket, confidence=fix_result.confidence)
        click.echo("\n--- MR Preview ---")
        click.echo(f"Branch: {mr_details['branch_name']}")
        click.echo(f"Title: {mr_details['title']}")
        click.echo(f"\nDescription:\n{mr_details['description']}")
    ...
    # Check for actual git modifications
    git_status = subprocess.run(
        ["git", "status", "--porcelain"],
        cwd=repo_path,
        capture_output=True,
