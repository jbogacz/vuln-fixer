...
def revert_repo_changes(repo_path: Path) -> bool:
    """Revert all uncommitted changes in the repo (git checkout -- . && git clean -fd)."""
    try:
        ...
        click.echo(f"Warning: Could not revert changes: {e}")
        return False
...
def build_mr_details(
    ...
    # Agent output is used directly as MR body (already formatted as markdown).
    # Fall back to a generic template if no explanation is available.
    if explanation and "## " in explanation:
        ...
        # Fallback: generic MR template (no structured output from agent)
        analysis_section = ""
        if explanation:
            analysis_section = (
                f"### Analysis\n"
...
def fix(ctx, vuln_id, jira_ticket, dry_run, force, local_only, skip_validation):
    ...
    # Step 4: NOW run Claude on the clean branch (with retry loop)
    max_retries = config.get("options", {}).get("max_retries", 1)
    retry_context = None
    ...
    for attempt in range(1 + max_retries):
        is_retry = attempt > 0
        if is_retry:
            click.echo(f"\n--- Retry {attempt}/{max_retries}: Re-running fix with validation feedback ---")
            revert_repo_changes(repo_path)

        click.echo("\nInvoking Claude CLI to fix...")
        if vuln.vuln_type == VulnType.DEPENDENCY:
            fix_result = agent.fix_dependency(vuln, repo_path, dry_run=False, retry_context=retry_context)
        else:
            fix_result = agent.analyze_and_fix(vuln, repo_path, dry_run=False, retry_context=retry_context)
        ...
        if attempt < max_retries:
            click.echo(click.style("\nValidation failed — retrying with feedback...", fg="yellow"))
            retry_context = FixerAgent.build_retry_context(validation)
        else:
            click.echo(click.style("\nValidation failed after retry!", fg="red"))
...
def build_group_mr_details(
    ...
    # List all affected files with vulnerability title
    files_list = "\n".join(
        f"- `{v.location.file_path}`: {v.title}" for v in vulns
    )
...
def fix_group(ctx, cve, jira_ticket, severity, dry_run, force, local_only, skip_validation):
    ...
    for attempt in range(1 + max_retries):
        ...
        click.echo("\nInvoking Claude CLI to fix...")
        fix_result = agent.fix_cve_group(cve, group, repo_path, dry_run=False, retry_context=retry_context)
